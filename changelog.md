...
- 修复 Linux/Windows 启动器打包超时：远程原生启动器工作流不再安装 Python 依赖或下载 runtime，仅上传原生二进制，避免无关的 Torch/CUDA 下载。
- 精简原生启动器 CI：删除 Python runtime、源码打包、SBOM/manifest 生成和依赖安装，仅执行三平台 CMake 编译、二进制检查、artifact 上传及 ClamAV 扫描。
